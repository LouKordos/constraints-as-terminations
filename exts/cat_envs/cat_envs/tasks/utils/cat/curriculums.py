...
def modify_constraint_p(env: ManagerBasedRLEnv, env_ids: Sequence[int], term_name: str, num_steps: int, init_max_p: float,):
    ...
    env.curriculum_manager.constraints_curriculum = min(env.curriculum_manager.constraints_curriculum + step_cur, 1.0)
    ...
    T_start = 20
    T_end = 1 / init_max_p
    init_max_p = 1 / (T_start + env.curriculum_manager.constraints_curriculum * (T_end - T_start))
