...
"""Matched upstream Isaac Lab rough-terrain baseline for ANYmal C."""
...
import cat_envs.tasks.utils.mdp.commands as cat_commands
import cat_envs.tasks.utils.mdp.events as cat_events
import cat_envs.tasks.utils.mdp.terminations as cat_terminations
...
@configclass
class MatchedAnymalCTerminationsCfg:
...
@configclass
class MatchedAnymalCCurriculumCfg:
...
def _assert_upstream_anymal_reward_contract(rewards) -> None:
    expected_weights = {
    ...
    for term_name, expected_weight in expected_weights.items():
        ...
        if actual_weight != expected_weight:
            raise ValueError(
                f"Upstream ANYmal C reward contract changed for {term_name}: "
...
class BaselineAnymalCRoughEnvCfg(AnymalCRoughEnvCfg):
    ...
    def __post_init__(self):
        ...
        _assert_upstream_anymal_reward_contract(self.rewards)
        ...
        # The CaT command implementation adds a 3-D deadzone, opportunistic
        # per-step resampling, and random yaw inversion; its override also does
        # not enforce the parent's is_standing_env mask. The switch changes only
        # the implementation and retains these matched command ranges.
        ...
        # clipping, and upstream height sign/offset remain unchanged. CaT's
        # custom height pose jitter is intentionally absent because this baseline
        # retains the upstream height function.
        ...
        self.events = MatchedAnymalCEventCfg()
        self.terminations = MatchedAnymalCTerminationsCfg()
        self.curriculum = MatchedAnymalCCurriculumCfg()
        self.scene.terrain.terrain_generator.curriculum = True
        self.scene.terrain.max_init_terrain_level = 1


@configclass
class BaselineAnymalCRoughEnvCfg_PLAY(BaselineAnymalCRoughEnvCfg):
    """One-environment ANYmal C baseline for matched quantitative evaluation."""
