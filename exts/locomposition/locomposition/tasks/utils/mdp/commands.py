...
class UniformVelocityCommandWithDeadzone(mdp.UniformVelocityCommand):
    ...
    def _update_command(self):
        ...
        if self.cfg.heading_command:
            ...
            self.vel_command_b[env_ids, 2] = torch.clip(self.cfg.heading_control_stiffness * heading_error, min=self.cfg.ranges.ang_vel_z[0], max=self.cfg.ranges.ang_vel_z[1],)
        ...
        self.vel_command_b[:, 2] *= (1 - 2 * torch.bernoulli(torch.full_like(self.vel_command_b[:, 2], p_ang_vel)).float())
