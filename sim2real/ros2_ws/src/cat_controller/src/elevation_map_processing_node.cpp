...
private:
    ...
    void process_and_publish_map()
    {
        ...
        if (shutdown_coordinator_.handle_exit_if_requested() ||
            time_utils::shutdown_if_deadline_exceeded("process_and_publish_map", last_processing_callback_time_,
                std::chrono::milliseconds{10 * processing_interval_}, shutdown_coordinator_))
        {
            return;
        }
        ...
        // I am aware that atomic shared ptrs are not lock-free, but as the control node will notie latency spikes that might occur due to the heap
        // allocs or waiting for a lock here, this is acceptable. If it becomes a real bottleneck, I will switch to double buffering.
