...
inline bool shutdown_if_deadline_exceeded(const std::string & caller, std::chrono::steady_clock::time_point & last_call_time,
    std::chrono::milliseconds allowed_threshold, ShutdownCoordinator & shutdown_coordinator)
{
    ...
    if (last_call_time != std::chrono::steady_clock::time_point{}) {
        ...
        if (delta > allowed_threshold) {
            shutdown_coordinator.shutdown(
                std::format("Duration threshold between consecutive callback executions exceeded in {}, allowed threshold={}ms, actual "
                            "elapsed duration={}ms, exiting.",
                    caller, allowed_threshold.count(), std::chrono::duration_cast<std::chrono::milliseconds>(delta).count()));
            return true;
        }
    }
    last_call_time = now;
