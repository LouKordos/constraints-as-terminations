...
def _load_robot_config():
    config_path = Path(get_package_share_directory("locomposition_bringup")) / "config" / "robot_real_go2.yaml"
...
def _launch_setup(context, *args, **kwargs):
    ...
    livox_launch_file = PathJoinSubstitution(
        [FindPackageShare("locomposition_state_estimation"), "launch", "livox.launch.py"]
    )
    ...
    odom_launch_file = PathJoinSubstitution(
        [FindPackageShare("locomposition_state_estimation"), "launch", "odom.launch.py"]
    )
    ...
    controller_launch_file = PathJoinSubstitution(
        [FindPackageShare("locomposition_controller"), "launch", "locomposition_control.launch.py"]
    )
    # Resolve the single pose topic that the controller health check should wait for, then pass it into
    # locomposition_controller/launch/locomposition_control.launch.py so that launch file does not need to know how Vicon vs odometry is selected.
    ...
    elevation_map_processing_launch_file = PathJoinSubstitution(
        [FindPackageShare("locomposition_controller"), "launch", "locomposition_elevation_map_processing.launch.py"]
    )
    ...
    elevation_map_comparison_launch_file = PathJoinSubstitution(
        [FindPackageShare("locomposition_controller"), "launch", "locomposition_elevation_map_comparison.launch.py"]
    )
...
def generate_launch_description():
    ...
    use_vicon_arg = DeclareLaunchArgument(
        ...
        description="Set whether Vicon or go2_odometry is used. Passed to locomposition_state_estimation/odom.launch.py and used here to resolve the controller pose topic.",
    )
    livox_publish_freq_arg = DeclareLaunchArgument(
        "livox_publish_freq",
        default_value=str(robot_config["livox_publish_freq"]),
        description="Livox publish frequency in Hz. Passed to locomposition_state_estimation/livox.launch.py.",
    )
    network_interface_arg = DeclareLaunchArgument(
        "network_interface",
        default_value=robot_config["network_interface"],
        description="Network interface used by the controller node. Passed to locomposition_controller/launch/locomposition_control.launch.py.",
    )

    base_frame_arg = DeclareLaunchArgument(
        "base_frame",
        default_value=robot_config["frames"]["base"],
        description="Robot base frame name. Passed to locomposition_state_estimation/odom.launch.py.",
    )
    livox_frame_arg = DeclareLaunchArgument(
        "livox_frame",
        default_value=robot_config["frames"]["livox"],
        description="Livox frame name. Passed to locomposition_state_estimation/livox.launch.py and locomposition_state_estimation/odom.launch.py.",
    )
    vicon_base_frame_arg = DeclareLaunchArgument(
        "vicon_base_frame",
        default_value=robot_config["frames"]["vicon_base"],
        description="Vicon base frame name. Passed to locomposition_state_estimation/odom.launch.py.",
    )
    ...
    lowstate_topic_arg = DeclareLaunchArgument(
        ...
        description="Low-level robot state topic. Passed to locomposition_controller/launch/locomposition_control.launch.py.",
    )
    livox_points_topic_arg = DeclareLaunchArgument(
        "livox_points_topic",
        default_value=robot_config["topics"]["livox_points"],
        description="Livox point cloud topic. Passed to locomposition_controller/launch/locomposition_control.launch.py.",
    )

    base_to_livox_x_arg = DeclareLaunchArgument(
        "base_to_livox_x",
        default_value=str(robot_config["transforms"]["base_to_livox"]["x"]),
        description="Base-to-Livox static transform X offset. Passed to locomposition_state_estimation/odom.launch.py.",
    )
    base_to_livox_y_arg = DeclareLaunchArgument(
        "base_to_livox_y",
        default_value=str(robot_config["transforms"]["base_to_livox"]["y"]),
        description="Base-to-Livox static transform Y offset. Passed to locomposition_state_estimation/odom.launch.py.",
    )
    base_to_livox_z_arg = DeclareLaunchArgument(
        "base_to_livox_z",
        default_value=str(robot_config["transforms"]["base_to_livox"]["z"]),
        description="Base-to-Livox static transform Z offset. Passed to locomposition_state_estimation/odom.launch.py.",
    )
    base_to_livox_yaw_arg = DeclareLaunchArgument(
        "base_to_livox_yaw",
        default_value=str(robot_config["transforms"]["base_to_livox"]["yaw"]),
        description="Base-to-Livox static transform yaw. Passed to locomposition_state_estimation/odom.launch.py.",
    )
    base_to_livox_pitch_arg = DeclareLaunchArgument(
        "base_to_livox_pitch",
        default_value=str(robot_config["transforms"]["base_to_livox"]["pitch"]),
        description="Base-to-Livox static transform pitch. Passed to locomposition_state_estimation/odom.launch.py.",
    )
    base_to_livox_roll_arg = DeclareLaunchArgument(
        "base_to_livox_roll",
        default_value=str(robot_config["transforms"]["base_to_livox"]["roll"]),
        description="Base-to-Livox static transform roll. Passed to locomposition_state_estimation/odom.launch.py.",
    )
