...
if [[ -z "${DOCKER_FLAG_FOR_RUN_SCRIPT}" ]]; then
    ...
    if [[ "$SKIP_MULTIARCH_BUILD" == false ]]; then
        ...
        COMPOSE_DOCKER_CLI_BUILD=0 DOCKER_BUILDKIT=0 docker compose --progress plain -f compose.yml -f compose.no-multiarch.yml build
    fi
    ...
    (
        ...
        set -x
        cd "${BAG_DIR}"
        exec ros2 bag record --output "${BAG_DIR}/bag" /tf /tf_static /joint_states /lowstate /lowcmd /pointcloud /robot_description /initialpose /imu_lowstate /utlidar/imu /imu /odom --regex "/odometry.*" > "${LOG_DIR}/ros2_bag_${BAG_TS}.log" 2>&1
    ) &
    BAG_PID=$!
