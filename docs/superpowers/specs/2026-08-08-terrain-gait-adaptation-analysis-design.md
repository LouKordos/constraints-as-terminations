...
4. if old and current full evaluations share the same checkpoint, prefer the candidate with explicit zero-delay metadata; gait-dynamics discovery independently requires its own non-empty payload;
